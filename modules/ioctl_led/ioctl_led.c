...
size_t My_write (struct file *mufile, const char __user *buff , size_t size , loff_t *conut)
    ...
    for(i = 0; i < size; i++) {
        if(kbuf[i] == '0') {
          //熄灭第i个灯：通过配置数据寄存器
            GPM4DAT  |=  1 << (i) ;  //把第i个IO口配置为高电平，
        }

        else if(kbuf[i ] == '1') {
           //点亮第i个灯：通过配置数据寄存器
            GPM4DAT  &= ~( 1 << (i));  //把第i个IO口配置为低电平，
        }
    }
  return 0;
	}
...
{       
     unsigned long pos;
     int ret=0;
     ret =  copy_from_user(&pos, (const void __user *)arg,1);
	if(ret){
	...
	switch (cmd)
	{
		case LED_ALLOFF:
			GPM4DAT  |=  0xf<<0 ;
			printk("led off arg = %ld \n",pos);
		break;
		case LED_ALLON:
			GPM4DAT  &= ~(0xf<<0);
			printk("led on arg = %ld \n",pos);
		break;
		case LEDx_ON:
			GPM4DAT  &= ~(0x1<<pos);
			printk("led arg = %ld \n",pos);
...
static int __init Cdev_driver_init(void)
{
   ...
   //5、自动创建设备名
   //给类指针赋值
   my_class =class_create(THIS_MODULE,"ioctl_led");
   if(IS_ERR(my_class)) {
